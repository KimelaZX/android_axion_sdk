...
public abstract class AxStackRecentsView<
    ...
    private void applyStackTransforms(boolean forceLiveTileUpdate) {
        ...
        for (int index = 0; index < childCount; index++) {
            ...
            boolean isReflowing = isTaskReflowingOnDismiss(taskView, mDismissDragTask);
            if (isReflowing) {
    ...
    private TaskView getDismissDragBehindTask(TaskView dismissedTask) {
        ...
        int lastTaskIndex = getLastStackTaskIndex();
    ...
    private int getLastStackTaskIndex() {
    ...
    private boolean isTaskReflowingOnDismiss(TaskView taskView, @Nullable TaskView dismissedTask) {
        if (dismissedTask == null || taskView == dismissedTask || !isStackTask(taskView)) {
            return false;
        }
        int dismissedIndex = indexOfChild(dismissedTask);
        int taskIndex = indexOfChild(taskView);
        if (dismissedIndex == INVALID_PAGE || taskIndex == INVALID_PAGE) {
            return false;
        }
        int lastTaskIndex = getLastStackTaskIndex();
        if (dismissedIndex < lastTaskIndex) {
            return taskIndex > dismissedIndex;
        }
        return taskIndex == dismissedIndex - 1;
    }
