...
import com.android.axion.customization.R
...
abstract class AxClockView @JvmOverloads constructor(
    ...
    open fun onDozeChanged(doze: Boolean) {
        interactor.onDozeChanged(doze)
        depthController.onDozeChanged(doze)
    }
    open fun onFidgetAnimation() {}
    open fun onChargeAnimation() {}
    open fun onPulsingChanged(doze: Boolean) {}
    open fun onScreenOff(screenOff: Boolean) {
        interactor.onScreenOff(screenOff)
        if (screenOff) {
            depthController.setDepthVisible(false)
        }
    }
    ...
    open fun onDozeAmountChanged(linear: Float, eased: Float) {
        ...
        depthController.onDozeAmountChanged(eased)
    }
